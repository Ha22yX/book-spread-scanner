  ...
  for (const block of page.blocks ?? [])
    for (const paragraph of block.paragraphs)
      for (const line of paragraph.lines) {
        ...
        // Single-column book mode can see background grain as tiny text lines.
        if (
          words.length &&
          (text.match(/[\p{L}\p{N}]/gu)?.length ?? 0) >= 3 &&
          line.confidence >= 40
        )
          spans.push({
...
export async function recognizeSpread(
  ...
  try {
    // Book pages are single columns; AUTO often fragments slanted body lines.
