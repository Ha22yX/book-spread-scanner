...
后台只查询前两张照片，不下载整本书的 OCR。分割、前文下载和原图下载并行，左右页图片并行下载，OCR 仍按左→右顺序运行。日志输出各阶段毫秒耗时，便于区分排队、网络、OCR 和 AI 延迟。相邻句子在同一 OCR 行上的定位合并为一个精确范围；最多 8 行。全部批注不符合长度或定位要求时，仅重新请求一次更短、单句定位的结果，仍不通过则明确失败，不无限重试。
...
- 通常整张双页照片只有一处批注，每条最多 10 个英文词，优先 4–8 词的简单短句或自然笔记短语，不为凑字数扩写。聚焦重要手法、人物变化或剧情；确有必要才增加一处，但每条仍不得超过 10 词。没有值得批注的内容时可以不生成。页边笔记通过引导线连接高亮，点击可定位。新规则适用于新生成和重新生成的批注，已有批注不自动改写。
